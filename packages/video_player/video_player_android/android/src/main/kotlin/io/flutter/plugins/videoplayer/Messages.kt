...
data class BufferConfigNative (
   ...
   */
  val bufferForPlaybackAfterRebuffer: Long
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): BufferConfigNative {
      val minBuffer = pigeonVar_list[0] as Long
      val maxBuffer = pigeonVar_list[1] as Long
      val bufferForPlayback = pigeonVar_list[2] as Long
      val bufferForPlaybackAfterRebuffer = pigeonVar_list[3] as Long
      return BufferConfigNative(minBuffer, maxBuffer, bufferForPlayback, bufferForPlaybackAfterRebuffer)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is BufferConfigNative) {
      return false
    }
    if (this === other) {
      return true
...
data class CreationOptions (
  ...
  val bufferConfig: BufferConfigNative
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): CreationOptions {
      ...
      val bufferConfig = pigeonVar_list[4] as BufferConfigNative
      return CreationOptions(uri, formatHint, httpHeaders, userAgent, bufferConfig)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
private open class MessagesPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      135.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          BufferConfigNative.fromList(it)
        }
      }
      136.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
